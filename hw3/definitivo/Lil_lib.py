...
def pred_rating_IB(item, user, closer, pivot):
    ...
    for it in closer.index:
        ...
        if somma<0:somma=0
        #lista[us]=[closer[us],pivot[us,item], round(mean(us,pivot),4)]
    z=somma/closer.sum()
    if z!=z:z=0
    return round(float(m+z),3) #lista
...
# 
# def pred_row_IB(user,closer, pivot):
#     userp={}
#     for i in range(pivot.shape[1]):
#         print(i)
#         if pivot[user,i]==0:
...
def rmse_mat(y,res):
    l=y.shape[0]
    return sqrt(((y.flatten()-res.flatten())**2).sum()/l)
